...
The main limitation: **a3 is the only control plane, and it runs on SQLite, not etcd.** If a3 goes down, running pods keep running, but nothing can be scheduled or changed until it is back. This is a deliberate trade-off. A three-node etcd control plane on consumer hardware over WiFi would add resilience I have not yet needed, at a cost in complexity I would pay every day. For now the single control plane is the right choice, but moving to HA is a planned next step — and the argument for it got a lot more concrete in September 2026, when [a kernel bug wedged a3 for nine hours](/hardware/nodes#a3--control-plane) and took the cluster API with it. Workloads on the other nodes kept running the whole time, which is exactly what k3s promises; what I could not do was change anything.

Adding a node is straightforward: run `scripts/install-k3s-server.sh` once for the server, then `scripts/install-k3s-agent.sh` for each agent. Even the arm64 DGX Spark on a different subnet joined without any special handling. The catch is that *joining* is the easy 10% — the token gets a node to `Ready`, but the host prep that keeps it healthy (disabling laptop sleep, WiFi power-save, inotify limits, LAN-CA trust, registry pinning, and since September 2026 the **kernel lockup guard**) is separate, and skipping it is silent. t430, the newest agent, joined `Ready` with almost none of that applied; see [The Six Machines](/hardware/nodes) for how that went. The prep scripts are all `scripts/node-*.sh` in the repo and the onboarding runbook is [`docs/20`](https://github.com/briancaffey/home-lab/blob/main/docs/20-node-onboarding-runbook.md).

## Nodes that reboot themselves

The lockup guard deserves its own paragraph because it changes what "a node is stuck" means here. Before it, a Linux kernel soft lockup on any node was silent and permanent: the kernel logged a complaint every few minutes and the machine sat there, hot and unreachable, until I noticed. Now every amd64 node has the kernel's lockup detector set to **panic**, a short panic timeout so the box reboots itself about 75 seconds after a core gets stuck, and the Intel chipset's hardware watchdog armed by systemd as a second line in case even PID 1 stops running. The trade is deliberate: an unplanned 2-minute reboot beats a 9-hour outage, and Kubernetes is built for exactly that kind of blip — pods reschedule, the node comes back `Ready`, and a `NodeRebooted` alert tells me it happened. The same script keeps `intel-microcode` installed and, with a flag, puts the node on the HWE kernel meta-package so it actually receives kernel updates. Details in [The Six Machines](/hardware/nodes#a3--control-plane) and [`docs/22`](https://github.com/briancaffey/home-lab/blob/main/docs/22-a3-thermal-runaway-postmortem.md).
