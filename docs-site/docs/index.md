...
- **Monitoring** — [Prometheus & Grafana](/observability/prometheus-grafana), [alerts to my phone](/observability/alerting), [disk-health monitoring](/observability/scrutiny), [LLM tracing with Langfuse](/observability/langfuse) plus [every other self-hostable OTLP backend the agent's tracing plugin supports](/observability/otel-backends), and [nightly backups with tested restores](/platform/backups).
