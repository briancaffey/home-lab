...
:::warning[🔥 War story]
给集群加了一个新节点后，它上面的 Pod 立刻出现在了 Loki 里（promtail 是 DaemonSet——它会自动落到每个节点上），但在 Dozzle 里却**完全看不见**。没有任何东西坏掉：Dozzle 在 Kubernetes 模式下*只在启动时枚举一次*节点，而这个节点是之后才加入的。它压根不知道这个节点存在。对 Dozzle 的 deployment 执行一次 `kubectl rollout restart`，新节点的日志就出现了。值得记住，因为它的失败方式是无声的——Dozzle 不会告诉你它给你看的是一个不完整的集群。
:::

```mermaid
flowchart LR
    PODS[所有 Pod] -->|stdout/stderr| PT[promtail ×6]
    PT --> LOKI[Loki——留存与搜索]
    LOKI --> G[Grafana]
